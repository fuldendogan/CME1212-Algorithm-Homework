...
public class Main {
    ...
    static Queue nameQ = new Queue(12);   // HighScore names maksimum 12
    static Queue scoreQ = new Queue(12);  // HighScore scores maksimum 12
    ...
    public static void main(String[] args) {
        try {
            ...
            while (myReader.hasNext()) {
                nameQ.enqueue(new String(myReader.next()));
                scoreQ.enqueue(new String(myReader.next()));
            }
            sortQueue(nameQ, scoreQ);
            System.out.println("q1: " + nameQ);
            System.out.println("q2: " + scoreQ);
...
//    }
    ...
    public static void sortQueue(Queue nameQ, Queue scoreQ) {
        int size = scoreQ.size();

        for (int i = 0; i < size - 1; i++) {
            for (int j = i + 1; j < size; j++) {
                Object scoreObj = scoreQ.peek();
                int score1;
                if (scoreObj instanceof Integer) {
                    score1 = (Integer) scoreObj;
                } else {
                    score1 = Integer.parseInt(scoreObj.toString());
                }                scoreQ.dequeue();
                String name1 = (String) nameQ.peek();
                nameQ.dequeue();

                Object scoreObj2 = scoreQ.peek();
                int score2;
                if (scoreObj instanceof Integer) {
                    score2 = (Integer) scoreObj;
                } else {
                    score2 = Integer.parseInt(scoreObj.toString());
                }                String name2 = (String) nameQ.peek();

                if (score1 > score2) {
                    scoreQ.enqueue(score1);
                    nameQ.enqueue(name1);

                    scoreQ.dequeue();
                    nameQ.dequeue();

                    scoreQ.enqueue(score2);
                    nameQ.enqueue(name2);
                } else {
                    scoreQ.enqueue(score2);
                    nameQ.enqueue(name2);
